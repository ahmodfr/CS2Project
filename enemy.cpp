...
void Enemy::findpath(int startingRow, int startingCol, int finalRow, int finalCol)
{
    ...
    else
    {
        ...
        Node currentNode(startingRow, startingCol, -1, -1, 0, getDistance(startingRow, startingCol, finalRow, finalCol));   //initializes starting Node that starts from the place of the enemy
        ...
        openNodes.push_back(currentNode);   // pushes it as the first step for the enemy to take

        while (!openNodes.empty())  // works until openNodes is empty
        {
            sort(openNodes);
